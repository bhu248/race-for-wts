...
def live_game_status_names(season, week, season_type="regular"):
    ...
    progress right now -- e.g. ["STATUS_IN_PROGRESS", "STATUS_HALFTIME"]
    for two concurrent live games, one of them at the half.
    ...
    _elapsed_fraction already relies on above -- but, unlike that
    function, does NOT collapse halftime into "in progress": a caller
    needing to tell "one game still being played" from "one game paused
    at the half" (local_scheduler.py, to decide whether to pause polling
...
def count_live_games(season, week, season_type="regular"):
    """
    How many NFL games are actually in progress right now -- used by
    ...
    not need to be checked as often. See live_game_status_names for the
    definition of "in progress" and the fail-safe posture on error.
    """
    return len(live_game_status_names(season, week, season_type))
